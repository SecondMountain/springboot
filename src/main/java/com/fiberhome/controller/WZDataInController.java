...
//        logger.info("本工程的资源目录为" + realPath);
        ...
        if (WZDataInService.getListByTime(time)!=null && WZDataInService.getListByTime(time).size()!=0){
            return "this date already exist";
        }
//        time="2017-12-18";
        Workbook workbook = excelProcess.getWorkBook(file);
        List<WZDataIn> list = excelProcess.importExcelContent2BeanList(workbook,0,2,WZDataIn.class,time);
...
//        String realPath = request.getSession().getServletContext().getRealPath("/");
//        logger.info("本工程的资源目录为" + realPath);
        return WZDataInService.getListByTime(time);
    }
